...
class ClassificationDataset:
    def __init__(self, dataset: str = "digit"):
        ...
        """
        assert dataset in ["digit", "fashion"], f"{dataset} is not supported"
        if dataset == "digit":
    ...
    def visualize(self, save_dir: str, n_samples: int = 16):
        ...
        train_path = os.path.join(save_dir, f'{self.dataset["name"]}_train_samples.png')
        utils.save_image(
        ...
        test_path = os.path.join(save_dir, f'{self.dataset["name"]}_test_samples.png')
        utils.save_image(
    ...
    def _get_digit(self):
        ...
        X_train, X_test, y_train, y_test = train_test_split(
            images, labels, test_size=0.3, random_state=42, shuffle=False
        )
        ds = {
            "train_data": X_train,
            "train_label": y_train,
